...
class Bot:
    ...
    def click(self, x, y, delay_mult=0.9):
    ...
    def try_merge(self, rank=1, prev_grid=None, merge_target='zealot.png'):
        ...
        if merge_target == 'demon_hunter.png':
            ...
            bards = adv_filter_keys(merge_series, units='bard,png')
            num_demon = sum(demons)
            num_bard = sum(bards)
            if num_bard >= 11:
                # If board is mostly bards, chill out
                self.logger.info(f'Board is full of bards, waiting...')
                time.sleep(10)
            if num_demon >= 11:
        ...
        merge_prio = adv_filter_keys(merge_series,
                                     units=['chemist.png', 'bombardier.png', 'summoner.png', 'knight_statue.png', 'priest.png'])
        ...
        if df_groups['empty.png'] <= 2:
            ...
            if not low_series.empty:
                ...
                merge_series = adv_filter_keys(merge_series,
                                               ranks=[3, 4, 5, 6, 7],
                                               units=['zealot.png', 'crystal.png', 'bruser.png', 'priest.png', merge_target],
                                               remove=True)
